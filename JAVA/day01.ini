   ...
   모니터, 마우스, 키보드, RAM, CPU : 하드웨어 

   게임, 음악프로그램, 웹 브라우저    : 소프트웨어 => 손으로 만질수 없고 
                  하드웨어의 도움이 필요한 애들
...
- 자바의 특징
   ...
   컴파일 : 프로그래밍 언어로 작성된 소스코드를 기계어로 변환하는 과정
      자바가 실행한다.

   저급언어 : 컴퓨터가 이해하기 쉬운 언어, 컴퓨터와 직접적인 언어
   ...
   3. 자바는 운영체제 독립적이다.
   자바의 큰 장점으로 다른 프로그래밍 언어로 만든 프로그램은 각각의 운영체제에 따라
   코드를 변경해줘야 하지만 자바는 "JVM"이 운영체제로부터 독립적으로
   자바 프로그램을 실행시켜주므로 하나의 프로그램을 만들어 놓으면
   여러 운영체제에서 코드 변경없이 사용이 가능하다.
...
- JDK 설치
   구글 -> JDK 17 Download 검색 -> 최상단 링크 클릭
   -> 하단 Java SE Development Kit 17.0.10 -> windows x64 installer
   -> 우측에 다운로드 버튼 클릭 -> 오라클 로그인 

- JAVA를 사용하기위해 필요한것
   1. JVM(Java Virture Machine)
